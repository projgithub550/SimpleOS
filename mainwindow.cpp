    ...
    , ui(new Ui::MainWindow)
{
    ...
    for() {
        DeviceDriver* device = new DeviceDriver;
        QThread *IOThread = new QThread;
        IOMap[device] = IOThread;
        device->moveToThread(IOThread);
        IOThread->start();

        connect(device, SIGNAL(tellIOExec()), device, SLOT(executePCB()));
        connect(device, SIGNAL(tellManFinIO(PCB* pcb)), procManager, SLOT(blocked2ready(PCB* pcb)));
    }
    // begin listen signal
    procManThread->start();
    cpuThread->start();
