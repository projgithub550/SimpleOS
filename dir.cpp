...
vector<pair<string, unsigned short>> DIR::os_ls() {
    ...
    for (int i = 0; i < DIR_FILE_NUM && FileManager::current_dir[i].file_name != "#"; i++) {
        pair<string, unsigned short> f = make_pair(FileManager::current_dir[i].file_name.toStdString(), FileManager::iNode_table[FileManager::current_dir[i].iNode_no].i_mode);
        files.push_back(f);
    }
    return files;
}
...
bool DIR::os_cd(string newpath) {
    ...
    if (temp == NULL) {
        if (newpath[0] == '/')
            qDebug() << "ERROR——找不到正确的绝对路径" << Qt::endl;
        else
            qDebug() << "ERROR——找不到正确的相对路径" << Qt::endl;
        return 0;
    }
    else {
